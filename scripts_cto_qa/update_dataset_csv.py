#!/usr/bin/env python3
"""将 CTO 单模型 3 次运行汇总写入数据集级 CSV（支持并行写入锁）。"""
import argparse
import csv
import fcntl
import json
...
def main():
    ...
    parser.add_argument("--method", default="CTO")
    ...
    with open(lock_path, "w", encoding="utf-8") as lock_f:
        ...
        key = (new_row["method"], new_row["dataset"], new_row["model_tag"])
        updated = False
        for i, row in enumerate(rows):
            if (row.get("method"), row.get("dataset"), row.get("model_tag")) == key:
                rows[i] = new_row
                updated = True
                break
        if not updated:
