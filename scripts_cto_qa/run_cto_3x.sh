#!/bin/bash
# 完整 CTO 流水线（step1→7），重复 NUM_RUNS 次并写入数据集级 CSV
# Step 3/5/7 使用 cto_guided_search.py（对比轨迹优化 + 经验检索）
...
_run_one_pipeline() {
  ...
  echo ""
  echo "========== CTO Run ${run_id}/${NUM_RUNS} | ${DATASET} × ${MODEL_TAG} =========="
