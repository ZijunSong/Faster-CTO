#!/bin/bash
# 断点续跑 RSE QA 流水线：已完成的 step 自动跳过，从未完成的 step 继续。
# 依赖 inc_common.sh 已设置 MODEL/DATASET 等变量。
# 可选环境变量 START_RUN（默认 1）：从第几次 run 开始检查。
...
_run_one_pipeline() {
  ...
  echo ""
  echo "========== Run ${run_id}/${NUM_RUNS} | ${DATASET} × ${MODEL_TAG} (resume) =========="
