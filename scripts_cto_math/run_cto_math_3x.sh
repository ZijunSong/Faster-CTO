#!/bin/bash
# 完整 CTO 数学流水线（step1→7），重复 NUM_RUNS 次并写入数据集级 CSV

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=inc_cto_math_common.sh
source "$SCRIPT_DIR/inc_cto_math_common.sh"
...
# TP>1 时禁用 vLLM custom all-reduce，避免 custom_all_reduce.cuh invalid argument
_VLLM_TP_ARGS=()
...
_run_one_pipeline() {
  ...
  echo ""
  echo "========== CTO Math Run ${run_id}/${NUM_RUNS} | ${DATASET} × ${MODEL_TAG} =========="
...
echo ""
echo "========== CTO Math 全部 ${NUM_RUNS} 次运行完成 =========="
